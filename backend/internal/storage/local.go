...
func (s *Local) SaveUserAvatar(file multipart.File, userID int64, ext string) (string, error) {
	if strings.TrimSpace(ext) == "" {
		ext = ".jpg"
	}
	rel := filepath.Join("avatars", fmt.Sprintf("%d", userID), "avatar-"+uuid.NewString()+ext)
	abs := filepath.Join(s.cfg.AssetsDir, rel)
	if err := os.MkdirAll(filepath.Dir(abs), 0o755); err != nil {
		return "", err
	}
	tmpPath := filepath.Join(s.cfg.TempDir, uuid.NewString()+".avatar")
	out, err := os.Create(tmpPath)
	if err != nil {
		return "", err
	}
