...
- 公共图书馆列表、筛选、详情、加入书架、公共上传入口。
- 私有/公共图书上传。
- 分类和标签读取、上传选择、列表筛选。
- 阅读器基础能力：元数据、目录、章节正文、章节切换、阅读设置、进度保存。
- 管理后台：概览、用户管理、公共图书审核、分类管理、标签管理、系统设置。
...
- 管理后台高级编辑能力仍可继续增强，例如用户资料编辑、物理删除公共图书开关、批量审核。
...
- 使用 `apiClient` 请求受保护资源时，注意释放 `URL.createObjectURL` 创建的对象 URL。
